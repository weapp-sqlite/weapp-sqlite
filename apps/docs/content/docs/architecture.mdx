...
六个平台都内置相同文件系统契约：使用 `getFileSystemManager()` 和 `env.USER_DATA_PATH` 持久化数据库。微信通过 `WXWebAssembly` 按代码包路径实例化；其余目标读取包内二进制后交给标准 WebAssembly。宿主能力不满足契约时返回结构化 `unsupported`，不会静默切换到内存。
